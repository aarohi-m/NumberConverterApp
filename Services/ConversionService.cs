        ...
        public string ConvertNumber(string value, string fromBase, string toBase)
        {
            try
            {
                ...
                Dictionary<string, Func<string, string>> conversionMethods = new()
                {
                    ...
                    { "decimal-to-binary", v => int.TryParse(v, out int n) ? Convert.ToString(n, 2) : "Invalid input" },
                    { "decimal-to-octal", v => int.TryParse(v, out int n) ? Convert.ToString(n, 8) : "Invalid input" },
                    { "decimal-to-hexadecimal", v => int.TryParse(v, out int n) ? Convert.ToString(n, 16) : "Invalid input" }
                };
